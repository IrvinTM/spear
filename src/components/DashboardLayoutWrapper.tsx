    ...
    <div className="relative z-10 flex min-h-screen w-full max-w-full overflow-x-hidden">
      ...
      <header className="fixed top-0 left-0 right-0 h-14 bg-stone-950/90 backdrop-blur-md border-b border-white/[0.08] z-30 flex items-center justify-between px-3 md:hidden">
        <div className="flex items-center gap-2.5 min-w-0">
          <button 
            onClick={toggle}
            className="w-9 h-9 flex items-center justify-center rounded-lg bg-stone-900/80 border border-white/[0.08] text-stone-300 hover:text-stone-50 transition-colors shrink-0 cursor-pointer"
            title="Abrir menú"
          >
            <Menu className="w-5 h-5" />
          </button>
          <div className="flex items-center gap-2 min-w-0">
            <div className="w-6 h-6 rounded-md bg-accent-500 flex items-center justify-center text-xs font-semibold text-white shrink-0">
              C
            </div>
      ...
      {collapsed && (
        <button 
          onClick={toggle}
          className="fixed top-6 left-6 z-40 w-10 h-10 hidden md:flex items-center justify-center rounded-xl bg-stone-900/80 backdrop-blur border border-white/[0.08] text-stone-300 hover:text-stone-100 hover:bg-stone-800 transition-all shadow-sm cursor-pointer"
        >
          <Menu className="w-5 h-5" />
        </button>
      )}
