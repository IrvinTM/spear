  ...
  return (
    <div className="bg-transparent flex flex-col h-full overflow-hidden">
      {expanded && (
        <>
          {/* Header */}
          <div className="p-4 border-b border-white/[0.06] flex items-center justify-between bg-stone-900/40">
            <div className="flex items-center gap-4">
              <div className={`relative w-12 h-12 rounded-full overflow-hidden border-2 flex-shrink-0 bg-stone-950/60 transition-colors duration-300 ${isTalking ? 'border-accent-400 shadow-[0_0_12px_rgba(45,111,245,0.2)]' : 'border-white/[0.12]'}`}>
                <div className="absolute inset-0 flex items-center justify-center">
                  {isTalking ? <AudioLines className="w-6 h-6 text-accent-300 animate-pulse" /> : <Bot className="w-6 h-6 text-stone-400" />}
                </div>
              </div>
              <div>
      ...
      <form onSubmit={handleSend} className={`p-3 bg-stone-900/40 ${expanded ? 'border-t border-white/[0.06]' : ''} flex gap-2 items-center`}>
        ...
        <button
          ...
          className="bg-accent-500 hover:bg-accent-400 disabled:opacity-40 disabled:hover:bg-accent-500 text-white px-4 py-2 rounded-lg font-medium text-sm transition-colors cursor-pointer"
        >
          Enviar
        </button>
      </form>
